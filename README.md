...
1. 选择今天或明天 Tab
2. 点击对应时间段
3. 输入 4 位房号（必须是合法小区房号）
4. 点击预定按钮

### 房号配置（重要）
**首次使用前必须配置合法房号列表**：

1. 编辑 `pages/booking/booking.js` 文件
2. 找到 `VALID_ROOM_NUMBERS` 数组（位于文件顶部）
3. 将数组中的房号替换为实际小区的有效房号列表
4. 房号列表来源：`room_numbers.txt` 文件（包含当前所有合法房号）
5. 重新编译并上传到微信开发者工具

> **注意**：房号合法性检查在前端代码中实现，如果房号不在 `VALID_ROOM_NUMBERS` 数组中，用户将收到"请输入 4 位正确小区房号"的提示，无法完成预定。每次小区房号变动时，必须同步更新此数组。
...
- 房号必须为 4 位数字
- 每人或每户人家每天最多可预定 3 个小时
- 取消预定需本人操作
- 需确保云函数触发器已启用
- **数据保留策略**：数据库只保留"今天"和"明天"两天的预定记录，每日 23:30 自动清理
- **房号列表维护**：`VALID_ROOM_NUMBERS` 数组必须与实际小区房号保持一致。房号列表存储在 `room_numbers.txt` 中，部署前需确认该文件内容准确，并在 `booking.js` 中同步更新。
