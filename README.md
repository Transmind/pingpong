...
4. 当前房号列表来源：`room_numbers.txt` 文件（包含当前所有合法房号）
5. 重新编译并上传到微信开发者工具

> **注意**：房号合法性检查在前端代码中实现，如果房号不在 `VALID_ROOM_NUMBERS` 数组中，用户将收到"请输入 4 位正确小区房号"的提示，无法完成预定。小区房号变动时，必须同步更新此数组。
...
- **房号列表维护**：按需修改`booking.js`文件里的`VALID_ROOM_NUMBERS` 数组，使其包含所有合法小区房号。
